...
/**
 * Fees smaller than this (in uudsh) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 UDSH.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-udsh-address
                    CScript scriptChange;
