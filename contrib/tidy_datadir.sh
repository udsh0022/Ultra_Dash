...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Ultradash Coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Ultradash Coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Ultradash Coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Ultradash Coin 0.7 datadir."
    ;;
  3)
    echo "Detected Ultradash Coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Ultradash Coin 0.8 datadir."
    ;;
esac
